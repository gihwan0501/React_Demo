...
export function useCreateBooking (key) {
    ...
    const mutation = useMutation(
        ...
        {
            onSuccess: (booking) => {
                queryClient.invalidateQueries(key); // key 값의 데이터를 무효화(이 데이터는 더이상 최신화 보장 불가)
                const bookings = queryClient.getQueryData(key) || [];   // key에 해당하는 캐시값을 가져오기 가능
                queryClient.setQueryData(key, [...bookings, booking]); // key값의 데이터를 변경
            }
        }
    );

    return {createBooking: mutation.mutate, isCreating: mutation.isLoading
    };
}
...
export function useUpdateBooking (key) {
    ...
    return {updateBooking: mutation.mutate, isUpdating: mutation.isLoading
    };
}
...
export function useDeleteBooking (key) {
    ...
    return {deleteBooking: mutation.mutate, isDeleting: mutation.isLoading
    };
}